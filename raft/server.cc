...
future<> server_impl::do_on_leader_with_retries(seastar::abort_source* as, AsyncAction&& action) {
    ...
    while (true) {
        if (as && as->abort_requested()) {
            throw request_aborted(format("Request aborted while performing action on current leader: {}, previous leader: {}", leader, prev_leader));
        }
        check_not_aborted();
        if (leader == server_id{}) {
    ...
    if (idx > _applied_idx) {
        ...
        if (as) {
            it->second.abort = as->subscribe([this, it] noexcept {
                it->second.promise.set_exception(request_aborted(format("Aborted while waiting to apply entry: {}", it->first)));
...
future<read_barrier_reply> server_impl::execute_read_barrier(server_id from, seastar::abort_source* as) {
    ...
    if (as && as->abort_requested()) {
        return make_exception_future<read_barrier_reply>(
            request_aborted(format("Abort requested before waiting for read barrier, read id is {} for commit idx {}", rid->first, rid->second)));
    }
    ...
    if (as) {
        read->abort = as->subscribe([this, read] noexcept {
            read->promise.set_exception(
                request_aborted(format("Abort requested while waiting for read barrier, read id is {} for commit idx {}", read->id, read->idx)));
            _reads.erase(read);
        });
