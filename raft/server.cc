...
future<> server_impl::wait_for_next_tick(seastar::abort_source* as) {
    ...
    try {
        ...
        throw request_aborted("Cancellation requested via abort_source::request_abort() while waiting for next tick");
    }
}
...
future<> server_impl::wait_for_leader(seastar::abort_source* as) {
    ...
    try {
        ...
        throw request_aborted("Cancellation requested via abort_source::request_abort() while waiting for leader");
...
future<> server_impl::wait_for_state_change(seastar::abort_source* as) {
    ...
    try {
        ...
        throw request_aborted("Cancellation requested via abort_source::request_abort() while waiting for state change");
...
future<bool> server_impl::trigger_snapshot(seastar::abort_source* as) {
    ...
    try {
        ...
        throw request_aborted("Cancellation requested via abort_source::request_abort() in snapshot trigger");
    } catch (seastar::broken_condition_variable&) {
        throw request_aborted("Condition variable _snapshot_desc_idx_changed is broken in snapshot trigger");
    }
...
future<> server_impl::wait_for_entry(entry_id eid, wait_type type, seastar::abort_source* as) {
    ...
    if (as && as->abort_requested()) {
        throw request_aborted(format("Abort requested while waiting for entry with idx: {}, term: {}", eid.term, eid.idx));
    }
    ...
    if (as) {
        it->second.abort = as->subscribe([it = it, &container, &eid] () noexcept {
            it->second.done.set_exception(request_aborted(format("Timed out waiting for idx: {}, term: {}", eid.term, eid.idx)));
            container.erase(it);
        });
...
future<entry_id> server_impl::add_entry_on_leader(command cmd, seastar::abort_source* as) {
    ...
    while (true) {
        ...
        try {
            ...
            throw request_aborted("Semaphore aborted while adding entry on leader");
        }
...
future<> server_impl::do_on_leader_with_retries(seastar::abort_source* as, AsyncAction&& action) {
    ...
    while (true) {
        if (as && as->abort_requested()) {
            throw request_aborted("Request aborted while doing on leader with retries");
        }
        check_not_aborted();
        if (leader == server_id{}) {
...
future<> server_impl::wait_for_apply(index_t idx, abort_source* as) {
    if (as && as->abort_requested()) {
        throw request_aborted("Abort requested while waiting for apply");
    }
    ...
    if (idx > _applied_idx) {
        ...
        if (as) {
            it->second.abort = as->subscribe([this, it] () noexcept {
                it->second.promise.set_exception(request_aborted("Timed out waiting for apply"));
...
future<read_barrier_reply> server_impl::execute_read_barrier(server_id from, seastar::abort_source* as) {
    ...
    if (as && as->abort_requested()) {
        return make_exception_future<read_barrier_reply>(request_aborted("Abort requested while executing read barrier"));
    }
    _reads.push_back({rid->first, rid->second, {}, {}});
    auto read = std::prev(_reads.end());
    if (as) {
        read->abort = as->subscribe([this, read] () noexcept {
            read->promise.set_exception(request_aborted("Abort requested while executing read barrier"));
            _reads.erase(read);
        });
...
future<> server_impl::set_configuration(config_member_set c_new, seastar::abort_source* as) {
    ...
    if (as) {
        _non_joint_conf_commit_promise->abort = as->subscribe([this] () noexcept {
            ...
            std::exchange(_non_joint_conf_commit_promise, std::nullopt)->promise.set_exception(
                request_aborted("Aborted while setting configuration"));
        });
    }
